			...
			<view class="goods-detail" v-if="list.length">
				<view class="order-item" v-for="(item, index) in list" :key="index">
					<view class='cell-group'>
						<view class='cell-item' @click="orderDetail(item.orderId)">
							<view class='cell-item-hd'>
								<view class='cell-hd-title'>订单编号：{{ item.orderId }}</view>
							</view>
					...
					<view class='img-list'>
						<view class='img-list-item' v-for="(goods, key) in item.items" :key="key">
							<image class='img-list-item-l little-img have-none' :src='goods.skuIamges' mode='aspectFill'></image>
							<view class='img-list-item-r little-right'>
								<view class='little-right-t'>
									<view class='goods-name list-goods-name' @click="orderDetail(item.orderId)">{{ goods.skuName }}</view>
								...
								<view class='goods-item-c'>
									<view class='goods-buy'>
										...
										<view class='goods-num'>× {{ goods.quantity }}</view>
									</view>
								</view>
							</view>
						</view>
					...
					<view class='cell-group'>
						<view class='cell-item'>
							<view class='cell-item-ft goods-num'>
								<text class='cell-ft-text'>合计<text class="red-price">￥ {{ item.totalPrice }}</text></text>
					...
					<view class='order-list-button'>
						...
						<button class='btn btn-circle btn-g' hover-class="btn-hover" @click="orderDetail(item.orderId)">查看详情</button>

						<button class='btn btn-circle btn-w' hover-class="btn-hover" v-if="item.status === 1 && item.payType === 0"
						 @click="toPay(item.orderId)">立即支付</button>

						<button class='btn btn-circle btn-w' hover-class="btn-hover" v-if="item.status === 1 && item.payType === 1 && item.ship_status === 3 && item.confirm === 1"
						 @click="tackDelivery(index)">确认收货</button>

						<button class='btn btn-circle btn-w' hover-class="btn-hover" v-if="item.status === 1 && item.payType === 1 && item.ship_status === 3 && item.confirm === 2 && item.is_comment === 1"
						 @click="toEvaluate(item.orderId)">立即评价</button>
			...
			orderList() {
				...
				this.$api.orderList(data, res => {
					if (res.status) {
						...
						if (res.code == this.status[this.tab]) {
			...
			formatOrderStatus(orderList) {
				orderList.forEach(item => {
					...
					if (item.status === 1) {
						...
						this.$set(item, 'order_status_name', '已关闭')
					}
					// 		break
					// 	case 2:
					// 		this.$set(item, 'order_status_name', '已完成')
