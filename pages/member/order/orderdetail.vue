<template>
	<view class="content">
		<view class="content-top">
				...
				<view class='cell-item add-title-item'>
					<view class='cell-item-bd'>
						...
						<view class="cell-bd-view">
							<text class="cell-bd-text">订单号：{{ orderInfo.orderId || ''}}</text>
							<button class='btn btn-g btn-small' hover-class="btn-hover" @click="copyData(orderInfo.orderId)">复制</button>
						</view>
						<view class="cell-bd-view">
							<text class="cell-bd-text">下单时间：{{ orderInfo.createTime || ''}}</text>
						</view>
						<view class="cell-bd-view" v-if="orderInfo.leftTime">
							<text class="cell-bd-text">过期时间：{{ orderInfo.leftTime || ''}}</text>
						</view>
					</view>
				</view>
			</view>
			...
			<view class='cell-group margin-cell-group'>
				...
				<view class='cell-item add-title-item' v-if="!orderInfo.store">
					<view class='cell-item-bd'>
						<view class="cell-bd-view black-text">
							<text class="cell-bd-text">收件人：{{ orderInfo.receiverName || ''}}</text>
						</view>
						<view class="cell-bd-view">
							<text class="cell-bd-text">{{ orderInfo.receiverArea|| ''}} {{orderInfo.receiverDetailAddress || ''}}</text>
						</view>
					</view>
				</view>
			</view>
			...
			<view class='img-list'>
				<view class='img-list-item' v-for="item in orderInfo.items" :key="item.id">
					<image class='img-list-item-l little-img have-none' :src='item.skuIamges' mode='aspectFill'></image>
					<view class='img-list-item-r little-right'>
						<view class='little-right-t'>
							<view class='goods-name list-goods-name' @click="goodsDetail(item.spuId)" >{{ item.skuName || ''}}</view>
							<!-- <view class='goods-name list-goods-name' @click="pintuanDetail(item.spuId)" v-else-if="orderInfo.order_type == 2">{{ item.skuName || ''}}</view> -->
						...
						<view class='goods-item-c'>
							<view class='goods-buy'>
								<view class='goods-salesvolume' v-if="item.addon !== null">{{ item.quantity || ''}}</view>
			...
			<view class='cell-group margin-cell-group order-price'>
				<view class='cell-item add-title-item'>
					...
					<view class='cell-item-ft'>
						<text class="cell-ft-p">￥{{ orderInfo.totalPrice || ''}}</text>
					</view>
				</view>
				<view class='cell-item add-title-item'>
					<view class='cell-item-bd'>
					...
					<view class='cell-item-ft'>
						<text class="cell-ft-p">￥{{ orderInfo.cost_freight || '0'}}</text>
					</view>
				</view>
				...
				<view class='cell-item add-title-item'>
					...
					<view class='cell-item-ft'>
						<text class="cell-ft-p red-price">￥{{ orderInfo.totalPrice || ''}}</text>
					</view>
				</view>
		...
		<view class="button-bottom">
			<button class='btn btn-circle btn-g' hover-class="btn-hover" v-if="orderInfo.status == 1" @click="cancelOrder(orderInfo.orderId)">取消订单</button>
			<button class='btn btn-circle btn-w' hover-class="btn-hover" v-if="orderInfo.status == 1" @click="toPay(orderInfo.orderId)">立即支付</button>
			<button class='btn btn-circle btn-w' hover-class="btn-hover" v-if="orderInfo.status == 3" @click="tackDeliery(orderInfo.orderId)">确认收货</button>
			<button class='btn btn-circle btn-w' hover-class="btn-hover" v-if="orderInfo.status == 4" @click="toEvaluate(orderInfo.orderId)">立即评价</button>
			<button class='btn btn-circle btn-w' hover-class="btn-hover" @click="customerService(orderInfo.orderId)" v-if="orderInfo.add_aftersales_status==true">申请售后</button>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goInvition() {
				uni.navigateTo({
					url: './invitation_group?order_id=' + this.orderInfo.orderId + '&close_time=' + this.teamInfo.close_time
				})
			},
			//拼团信息
			getTeam(id) {
