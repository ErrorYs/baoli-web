				...
				<scroll-view class="goods-content" scroll-y="true">
					...
					<view class="goods-item">
						<view class="goods-item-box" v-if="isChild">
							<view v-for="(item, index) in beans[ins].children" :key="index">
								<view class="cate-title">{{item.name}}</view>
								<view class="goods-items" v-for="(value, inde) in item.children" :key="value.id"  @click="goClass(value.id)">
									<image class="goods-item-img"  :src="value.images" alt="" mode="aspectFill" />
									<view class="goods-item-name">{{value.name}}</view>
								</view>
							</view>
						</view>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			active(index) {
				this.ins = index;
				this.isChild = this.beans[index].hasOwnProperty('children');
			},
			categories() {
				this.$api.categories({}, res => {
					if (res.status) {
						...
						this.beans = res.data;
						this.isChild = this.beans[0].hasOwnProperty('children');
					}
				});
			},
			goClass(cat_id) {
