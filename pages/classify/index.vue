<template>
	<view class="content">
		...
		<view class="screen">
			...
			<view class="screen-item" @click="priceSort">
				...
				<view class="screen-item-icon">
					...
					<image v-else-if="!(searchData.order.key == 'price' && searchData.order.sort == 'asc')" class="screen-item-icon-img"
					 src="/static/image/top-gray.png"></image>
					...
					<image v-if="!(searchData.order.key == 'price' && searchData.order.sort == 'desc')" class="screen-item-icon-img"
					 src="/static/image/bottom-gray.png"></image>
				</view>
			</view>
			<view class="screen-item" @click="salesVolume">
				<text class="screen-item-text">销量</text>
				<view class="screen-item-icon">
					<image v-if="searchData.order.key == 'sold' && searchData.order.sort == 'asc'" class="screen-item-icon-img" src="/static/image/top-black.png"></image>
					<image v-else-if="!(searchData.order.key == 'sold' && searchData.order.sort == 'asc')" class="screen-item-icon-img"
					 src="/static/image/top-gray.png"></image>
					<image v-if="searchData.order.key == 'sold' && searchData.order.sort == 'desc'" class="screen-item-icon-img" src="/static/image/bottom-black.png"></image>
					<image v-if="!(searchData.order.key == 'sold' && searchData.order.sort == 'desc')" class="screen-item-icon-img"
					 src="/static/image/bottom-gray.png"></image>
				</view>
			</view>
		...
		<lvv-popup position="top" ref="lvvpopref" style="background: none;">
			<view class="fliter-c">
				<scroll-view scroll-y="true" style="height: 100%;">
					...
					<!-- //规格 -->
					<view class="fliter-item" v-for="value in specList" :key="value.key" v-if="specList.length > 0">
						<view class='cell-item right-img'>
							<view class='cell-item-hd'>
								<view class='cell-hd-title'>{{value.key}}</view>
							</view>
						</view>
						<view class="fliter-i-c">
							<view v-for="item in value.options" :key="item" v-if="item" @click="selectSpec(value.key,item)">
								<view class="fic-item" v-if="!specSelect(value.key,item)">
									<view class="fic-item-text two-line">
										{{item}}
									</view>
								</view>
								<view class="fic-item fic-item-active" v-else-if="specSelect(value.key,item)">
									<view class="fic-item-text two-line">
										{{item}}
									</view>
								</view>
		...
		<!-- 商品列表 -->
		<scroll-view scroll-y="true" :scroll-into-view="toView" class="scroll-Y" @scrolltolower="lower" enable-back-to-top="true"
		 lower-threshold="45">
			<!-- 表格图片 -->
					...
					<view class="img-grids-item" v-for="(item, index) in goodsList" :key="index" @click="goodsDetail(item.id)">
						...
						<view class="img-grids-item-b">
							...
							<view class="goods-item-c">
								...
								<view class="goods-price red-price">￥{{item.price}} <span class="old-price">￥{{item.originalPrice}}</span></view>
						...
						<view class="img-list-item-r">
							...
							<view class="goods-item-c">
								...
								<view class="goods-buy">
									<!-- <view class="goods-salesvolume">123</view> -->
		...
		methods: {
			...
			selectSpec(key, value) {
				...
				if (this.specSearch.length > 0) {
					let index = -1
					for (var i = 0; i < this.specSearch.length; i++) {
						for (let k in this.specSearch[i]) {
							if (key === k) {
