<template>
	<form class="content" @submit="toPay" report-submit="true">
		<view class="content-top">
						...
						<view class='cell-item add-title-item right-img'>
							...
							<view class='cell-item-bd'>
								<view class="cell-bd-view">
									...
									<text class="cell-bd-text-right">{{ userShip.phone || ''}}</text>
								</view>
								<view class="cell-bd-view">
									<text class="cell-bd-text address">{{ userShip.areaName || ''}}</text>
								</view>
							</view>
			...
			<!-- 商品列表信息 -->
			<view class='img-list'>
				<view class='img-list-item'  v-for="(item, index) in products" :key="index" v-if="item.selected">
					...
					<view class='img-list-item-r little-right'>
						...
						<view class="romotion-tip" v-if="item.promotion_list">
							<view class="romotion-tip-item" :class="v.type !== 2 ? 'bg-gray' : ''" v-for="(v, k) in item.promotion_list" :key="k">
								{{ v.name || ''}}
							</view>
						</view>
						<view class='goods-item-c'>
							<view class='goods-buy'>
								<view class='goods-salesvolume' v-if="item.skuParam !== null">{{ item.skuParam || ''}}</view>
			...
			<view class='cell-group'>
				...
				<view class='cell-item'>
					...
					<view class='cell-item-ft'>
						<view class="cell-ft-view red-price">{{ cartData.amount || ''}}</view>
			...
			<view class='cell-group leave-message'>
				...
				<view class="cell-textarea ">
					<!-- #ifdef MP-WEIXIN -->
					<input v-model="note" placeholder="50字以内(选填)"/>
					<!-- #endif -->
					<!-- #ifndef MP-WEIXIN -->
					<textarea v-model="note" placeholder="50字以内(选填)" maxlength="50"/>
					<!-- #endif -->
				</view>
			</view>
		</view>
    ...
    export default {
		...
		onShow() {
			...
			if (user_ship) {
				...
				this.params.areaId = user_ship.areaId;
