			...
			<view class='swiper'>
				<swiper class="swiper-c" :indicator-dots="swiper.indicatorDots" :autoplay="swiper.autoplay" :interval="swiper.interval"
				 :duration="swiper.duration">
					<swiper-item class="have-none" v-for="(item, index) in goodsInfo.bannerImage" :key="index" @click="clickImg(item)">
			...
			<view class='cell-group'>
				<view class='cell-item goods-top'>
					<view class='cell-item-hd'>
						<view class='cell-hd-title goods-price red-price'>￥{{ selectSku.price || '' }}</view>
						<view class='cell-hd-title goods-price cost-price' v-if="parseFloat(selectSku.originalPrice)>0">￥{{ selectSku.originalPrice  || ''}}</view>
					</view>
					<view class='cell-item-ft'>
						<text>{{ selectSku.skuStock.sold || '' }} 人已购买</text>
					</view>
				</view>
				...
				<view class='cell-item goods-details'>
					<view class='cell-item-hd'>
						<view class='cell-hd-title'>
							...
							<text v-if="goodsInfo.subTitle" class="color-9 fsz24 " v-html="goodsInfo.subTitle">
								<!-- {{ goodsInfo.subTitle || '' }} -->
							</text>
						</view>
					</view>
				...
				<view class='cell-item goods-title-item' >
					...
					<view class='cell-item-bd' @click="toshow()">
						<text class='cell-bd-text'>{{ selectSku.ownParam || ''}}</text>
					</view>
				</view>
				<!-- 规格end -->
			...
			<view class="goods-content">
				...
				<view class="goods-content-c">
					<view class="goods-detail" v-show="current === 0">
						<jshopContent :content="goodsInfo.spuDetail.description" v-if="goodsInfo.spuDetail.description"></jshopContent>
				...
				<view class="pop-c">
					<view class="pop-t">
						<view class='goods-img'>
							<image :src='selectSku.images' mode='aspectFill'></image>
						</view>
						<view class='goods-information'>
							<view class='pop-goods-name'>{{ selectSku.name || ''}}</view>
							<view class='pop-goods-price red-price'>￥ {{ selectSku.price || ''}}</view>
						</view>
					...
					<scroll-view class="pop-m" scroll-y="true" style="max-height: 560upx;">
						<spec :spesData="goodsInfo.spec" :indexes="JSON.parse(selectSku.indexes)" ref="spec" @changeSpes="changeSpes"></spec>
						<view class="goods-number">
							<text class="pop-m-title">数量</text>
							<view class="pop-m-bd-in">
								<uni-number-box :min="minNums" :max="selectSku.stock" :value="buyNum" @change="bindChange"></uni-number-box>
							</view>
						</view>
					</scroll-view>
					<view class="pop-b">
						...
						<button class='btn btn-square btn-b btn-all' hover-class="btn-hover2" @click="clickHandle()" :disabled='submitStatus'
						 :loading='submitStatus' v-if="selectSku.skuStock.stock">确定</button>
		...
		methods: {
			...
			getGoodsDetail() {
				...
				this.$api.goodsDetail(data, res => {
					if (res.status == true) {
						let info = res.data;
						let products = res.data.skuList;
						this.selectSku = products[0]
						this.goodsParams = res.data.baseParam
			...
			changeSpes(obj) {
				for (var i = 0; i < this.product.length; i++) {
					if(this.product[i].indexes==obj){
						this.selectSku = this.product[i]
					}
				}
				this.buyNum = this.selectSku.skuStock.stock > this.minBuyNum ? this.minBuyNum : this.selectSku.skuStock.stock
				// let index = obj.v;
				// let key = obj.k;
